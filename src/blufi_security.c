/*
   Code was borrowed from the ESP_IDF example code.
...
static const char *BLUFI_SEC_TAG = "BLUFISEC";
...
void blufi_dh_negotiate_data_handler(uint8_t *data, int len, uint8_t **output_data, int *output_len, bool *need_free)
{
    ...
    if (blufi_sec == NULL) {
        ESP_LOGE(BLUFI_SEC_TAG,"BLUFI Security is not initialized");
    ...
    case SEC_TYPE_DH_PARAM_LEN:
        ...
        if (blufi_sec->dh_param == NULL) {
            ...
            ESP_LOGE(BLUFI_SEC_TAG,"%s, malloc failed\n", __func__);
            return;
        }
        break;
    case SEC_TYPE_DH_PARAM_DATA:{
        if (blufi_sec->dh_param == NULL) {
            ESP_LOGE(BLUFI_SEC_TAG,"%s, blufi_sec->dh_param == NULL\n", __func__);
        ...
        if (ret) {
            ESP_LOGE(BLUFI_SEC_TAG,"%s read param failed %d\n", __func__, ret);
        ...
        if (ret) {
            ESP_LOGE(BLUFI_SEC_TAG,"%s make public failed %d\n", __func__, ret);
