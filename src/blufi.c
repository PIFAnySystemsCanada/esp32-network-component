...
void set_sta_connected(bool status) 
{
    gl_sta_connected = status;
}

void set_sta_bssid(uint8_t *sta_bssid)
{
    memcpy(gl_sta_bssid, sta_bssid, 6);
}
...
static void blufi_event_callback(esp_blufi_cb_event_t event, esp_blufi_cb_param_t *param)
{
    ...
    case ESP_BLUFI_EVENT_REQ_CONNECT_TO_AP:
        ESP_LOGI(BLUFI_TAG, "BLUFI requset wifi connect to AP\n");
...
*/
    ...
    if (ret) {
        ESP_LOGE(BLUFI_TAG, "%s initialize bt controller failed: %s\n", __func__, esp_err_to_name(ret));
    }

    ret = esp_bt_controller_enable(ESP_BT_MODE_BLE);
    if (ret) {
        ESP_LOGE(BLUFI_TAG, "%s enable bt controller failed: %s\n", __func__, esp_err_to_name(ret));
        return;
    }

    ret = esp_blufi_host_and_cb_init(&blufi_callbacks);
    if (ret) {
        ESP_LOGE(BLUFI_TAG, "%s initialise failed: %s\n", __func__, esp_err_to_name(ret));
        return;
    }

    ESP_LOGI(BLUFI_TAG, "BLUFI VERSION %04x\n", esp_blufi_get_version());
}
